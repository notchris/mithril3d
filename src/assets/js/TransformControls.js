...
var TransformControls = function ( camera, domElement, raycastLayer, useFar) {
	...
	ray.layers.enable(raycastLayer ? raycastLayer : 0);
	...
	this.pointerHover = function ( pointer ) {
		...
		if (useFar) {
			...
			camera.localToWorld(ray.ray.origin);
		}
		ray.setFromCamera( pointer, this.camera );
		if (useFar) {
			ray.ray.origin.z = camera.far;
		}
		...
		for ( var i = 0; i < handles.length; i ++ ) {
			...
			switch (this.camera.name) {
				case 'a':
					...
					break;
				case 'b':
					handle.position.set( this.worldPosition.x, 0, this.worldPosition.z );
					handle.scale.set( 60, 60, 60 )
					break;
				case 'c':
					handle.position.set( 0, this.worldPosition.y, this.worldPosition.z );
					handle.scale.set( 60, 60, 60 )
					break;
				case 'd':
					handle.position.set( this.worldPosition.x, this.worldPosition.y, 0);
					handle.scale.set( 60, 60, 60 )
					break;
				default:
					break;
